...
{
    internal class cls_16_MainCablesFromCT
    {
        public ProcessResult MainCablesFromCt(
            string projectCode,
            string excelPath,
            string projectUnits
        )
            ...
            using (DocumentLock docLock = doc.LockDocument())
            {
                ...
                using (Transaction tr = db.TransactionManager.StartTransaction())
                {
                    try
                    {
                        ...
                        // Obtenemos info
                        int? totalLabelsCreated = cls_16_ProcessCablesFromCT.
                            ProcessCablesFromCT(ed, db, tr, btr, excelPath, projectUnits);
                        // Validamos
